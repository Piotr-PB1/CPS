...
def bandpass_from_lowpass(h_lp, f_center, fs):
    M = len(h_lp)
    mid = (M - 1) / 2
    n = np.arange(M)
    h_bp = 2 * h_lp * np.cos(2 * np.pi * f_center * (n - mid) / fs)
    return h_bp


def design_filter(M, K=None, window_type='rectangular', filter_type='lowpass',
                  f_center=None, fs=None, bandwidth=None):
    if filter_type == 'lowpass':
        if K is None:
            raise ValueError("Dla filtru lowpass wymagany parametr K")
        h = lowpass_filter_coefficients(M, K)
        h = apply_window(h, window_type)
        return h
    elif filter_type == 'bandpass':
        if f_center is None or fs is None or bandwidth is None:
            raise ValueError("Dla filtru bandpass wymagane f_center, fs, bandwidth")
        K_lp = 2 * fs / bandwidth
        h_lp = lowpass_filter_coefficients(M, K_lp)
        h_lp = apply_window(h_lp, window_type)
        h_bp = bandpass_from_lowpass(h_lp, f_center, fs)
        return h_bp
    else:
